...
1. **`bypass_actors` está vazio de propósito.** Não chutei o `actor_id` do papel de
   ...
   ninguém pediu. Depois de importar, se quiser poder mergear com CI vermelho em
   emergência: **Edit ruleset → Bypass list → Add bypass → Repository admin**.

   Sem isso, nem o dono do repositório escapa das regras. Para um projeto de um
   desenvolvedor só, isso é uma decisão real: protege contra o próprio descuido, mas
   não deixa saída se o CI quebrar por causa externa.

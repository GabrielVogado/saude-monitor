...
| RN-10 | **Estrutura do formulário (máx. 4 perguntas)** | ① Você foi atendido? *(Sim, fui atendido / Sim, mas desisti / Não fui atendido)* — se "Não fui atendido", pula para a nota geral; ② Teve médico disponível? *(Sim / Não / Não precisei)*; ③ Fez triagem? *(Sim / Não / Não sei)*; ④ Como você avalia o atendimento? *(Nota 1–5 com estrelas)*. Ao final: campo **opcional** de comentário e pergunta sobre medicação/receita em formato de chips rápidos (Sim, recebi / Não recebi / Não precisei). |
| RN-11 | **Pular** | Todas as perguntas são **puláveis**. O envio é possível com pelo menos a nota geral respondida. |

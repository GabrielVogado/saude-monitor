# 🌳 Árvore Tecnológica — saude-monitor v2.1
...
> |---|---|
> | **Versão** | 2.1 |
> | **Status** | Proposta técnica — revalidada contra a branch `feature/mapbox-migration` (12/09/2026) |
...
> | **O que mudou na v2.1** | Mapa mobile: `react-native-maps` (registrado aqui, nunca usado no código) → `@maplibre/maplibre-react-native` (código até 11/09/2026) → **`@rnmapbox/maps` v10** (estilo `Street`, token `EXPO_PUBLIC_MAPBOX_TOKEN`, sem Expo Go). Backend: novo contexto `regiao` servindo as 4 camadas geográficas do classpath. Versão anterior (2.0) preservada em `_historico/Arvore-Tecnologica-v2.0.md`. |
...
graph TD
    subgraph CLIENTE["📱 Cliente (Mobile — React Native/Expo)"]
        ...
        APP --> MAP["@rnmapbox/maps v10 (mapa e polígonos)"]
...
| **Mapa mobile (`@rnmapbox/maps` v10)** | ✅ **MANTER** | Necessário para mapa de hospitais, polígonos de geofence e exibição da posição. Linhagem: `react-native-maps` (só existiu nestes documentos) → MapLibre (código até 11/09/2026, sem token) → Mapbox (estilo `Street`; exige token público `EXPO_PUBLIC_MAPBOX_TOKEN` e build nativo — sem Expo Go; repo Maven declarado em `frontend/android/build.gradle`). |

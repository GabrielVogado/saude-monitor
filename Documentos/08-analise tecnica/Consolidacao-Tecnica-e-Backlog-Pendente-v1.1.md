...
| ARQ-05 | ADR-003 / Prob. 5 — listas sem memoização | ✅ **Corrigido (03/09/2026)** | Os 4 sítios fechados. O diagnóstico original estava incompleto: `React.memo` sozinho não resolveria nada, porque os `renderItem` inline criavam `onPress={() => abrirDetalhe(item)}` a cada render — prop nova por identidade, memo errando sempre. Foi preciso **mudar o contrato dos componentes de linha**: `CSHospitalCard` e o novo `LinhaHospital` devolvem o próprio item ao chamador, e os handlers das telas foram estabilizados com `useCallback`. Também entraram dois achados do `code-review`: o `ListEmptyComponent` não memoizado remontava a subárvore inteira a cada tecla numa busca sem resultado, e `desabilitado={enviandoId !== null}` virava em todas as linhas, anulando o memo justamente durante a interação. **FlashList segue adiado**, como previsto | 2–3h | **entregue** |
| — | *Ressalva do ARQ-05* | ⚠️ | Dois dos 4 sítios estão em **código morto**: `CheckinManualScreen` não está registrada em nenhum navigator (a rota saiu na revisão de navegação, quando o check-in manual migrou para o `CSHospitalCard`) e `CSSelect` não tem um único consumidor. A memoização deles não é verificável por teste nem exercitável no app. Candidatos a remoção — decisão do PO | — | — |

...
| ENG-05 | 3 telas sem teste unitário | `SugerirHospitalScreen` (2,85%), `RevisarSugestaoScreen` (3,03%), `UserScreen` (3,22%) — e `PrivacidadeScreen` a 16,66% | **Atualizado em 04/09/2026 (Onda 1 de cobertura):** a `LoginScreen` saiu da lista — era o pior número do projeto (0%) e o exemplo citado nominalmente pelo PO; foi a **100%** nas quatro métricas, com 9 testes cobrindo os quatro caminhos do `handleLogin`. A `SugestoesPendentesScreen` (77,77%) e a `HomeScreen` (80%) também saíram do zero, no E8-13. Entrou a `UserScreen`, que a lista anterior não registrava. Em 03/09 a `CheckinManualScreen` saiu porque o arquivo foi **removido** — era tela órfã, sem rota. Componentes: **12 de 12 acima de 90% em statements, functions e lines** — mas em *branches* a pasta está em **76,19%**, e só o `CSTextField` passa de 90 (CSLoading 25%, CSBadge 40%, CSRatingStars 58,8%, CSCard/CSChip 66,7%, CSEmptyState 70%, CSHeader/CSIconButton 75%, CSHospitalCard 77,8%, CSButton 88,9%). E `components/index.js` está em **0/0/0/0**: o `barrel.test.js` lê o arquivo com `fs.readFileSync` e nunca o importa. A régua do PO é 90% **nas quatro métricas**, então os componentes ainda não a cumprem · **Lição da onda:** o `code-review` provou por mutação que 3 dos 19 testes novos passavam com o código de produção quebrado — inclusive o que levava a `LoginScreen` a 100% de *branches* sem ler a prop que dizia proteger. Corrigidos antes do merge. Cobertura só conta quando o teste falha se o comportamento quebrar |

...
|---|---|---|---|
| ENG-01 | ~~**O CI não roda os testes do frontend**~~ — ✅ **Resolvido em 02/09/2026 (E8-12)** | O job de frontend do `ci.yml` passou a executar o step `Testes (Jest)` (`npm test -- --ci --maxWorkers=2`); os **171 testes** rodam em todo `push`/`pull_request` para `develop` e `master` (até 03/09/2026 o gatilho dizia `main`, branch que nunca existiu — ou seja, **nada rodava na branch de produção**; corrigido na P-004) | Gate ativo e **obrigatório**: `develop` foi protegida em 02/09/2026 exigindo os checks `Backend (Spring Boot)` e `Frontend (Expo Web)`. Sem revisão obrigatória e com bypass de administrador permitido, para não travar o fluxo de um único mantenedor |
...
| ENG-05 | 3 telas sem teste unitário | `SugerirHospitalScreen` (2,85%), `RevisarSugestaoScreen` (3,03%), `UserScreen` (3,22%) — e `PrivacidadeScreen` a 16,66% | **Atualizado em 04/09/2026 (Onda 1 de cobertura):** a `LoginScreen` saiu da lista — era o pior número do projeto (0%) e o exemplo citado nominalmente pelo PO; foi a **100%** nas quatro métricas, com 11 testes cobrindo os quatro caminhos do `handleLogin`. A `SugestoesPendentesScreen` (77,77%) e a `HomeScreen` (80%) também saíram do zero, no E8-13. Entrou a `UserScreen`, que a lista anterior não registrava. Em 03/09 a `CheckinManualScreen` saiu porque o arquivo foi **removido** — era tela órfã, sem rota. Componentes: os **11** da pasta em **100% de statements, functions e lines** — mas em *branches* a pasta está em **76,19%**, e só o `CSTextField` passa de 90 (CSLoading 25%, CSBadge 40%, CSRatingStars 58,8%, CSCard/CSChip 66,7%, CSEmptyState 70%, CSHeader/CSIconButton 75%, CSHospitalCard 77,8%, CSButton 88,9%). O `components/index.js` **não entra nessa conta e não é lacuna**: módulo só de reexportação, com `total: 0` nas quatro métricas no `coverage-summary.json` — não há statement a cobrir, e o `0 | 0 | 0 | 0` da tabela do Jest é como ele imprime "nada a medir". É verdade que o `barrel.test.js` lê o arquivo com `fs.readFileSync` sem importá-lo, mas não é essa a causa do número: 7 telas de produção importam o barril e seus testes rodam. A régua do PO é 90% **nas quatro métricas**, então os componentes ainda não a cumprem · **Lição da onda:** o `code-review` provou por mutação que testes novos passavam com o código de produção quebrado — 3 na primeira passada (inclusive o que levava a `LoginScreen` a 100% de *branches* sem ler a prop que dizia proteger), 7 na segunda e **mais 1 na auditoria de 04/09/2026**: "digitar propaga o texto ao chamador" sobrevivia à remoção de `onChangeText={onChangeText}` do `TextInput`, porque o `fireEvent` do RNTL sobe a árvore e acha o handler no elemento composto. Todos corrigidos antes do merge. Cobertura só conta quando o teste falha se o comportamento quebrar |

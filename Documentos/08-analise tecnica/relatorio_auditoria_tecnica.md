...
>
> **Commit auditado:** a v3.0 não declarava contra qual estado do repositório foi escrita — o que a tornou impossível de reverificar. Esta revisão foi conferida contra **`develop@f26666e`** (após a Sprint S8). Toda auditoria futura deve declarar o commit-base no cabeçalho.
>
> **Três afirmações da v3.0 não se sustentaram na reverificação** e estão corrigidas em linha, sinalizadas com ⚠️ **Correção 3.1**:
>
> | # | Afirmação da v3.0 | Verificação em `develop@f26666e` |
> |---|---|---|
> | 1 | `CSGeoStatusCard.js ← Dependência de GeofencingTaskService` (árvore de diretórios) | **Falsa.** O componente não importa `GeofencingTaskService` nem qualquer serviço |
> | 2 | "O topo da hierarquia de dependências de `CSGeoStatusCard` inclui `@maplibre/maplibre-react-native`" | **Falsa.** `CSGeoStatusCard.js` importa apenas `react`, `react-native`, `CSCard`, `CSButton` e `../theme`. O argumento sobre *barrel exports* permanece válido; **este exemplo específico, não** |
> | 3 | "A pasta `src/services/` contém apenas um arquivo: `TokenStorage.js`" | **Desatualizada.** Contém três: `TokenStorage.js`, `DispositivoId.js` e `NotificacaoPermissao.js` (este último criado na S8) |
>
> **O que a auditoria não cobriu e é hoje o problema mais grave:** a v3.0 avaliou arquitetura, tipagem e organização de código, mas **não mediu o sistema em execução**. A medição feita em 02/09/2026 mostra **109 s de latência na primeira abertura (HTTP 503)** e **1 a 5 s por requisição** com o serviço quente — origem na instância do backend (`plan: free` no Render), não no código do aplicativo. Nenhuma das 11 recomendações deste relatório resolve isso, e a prioridade delas fica **abaixo** da Sprint S9. Ver `Consolidacao-Tecnica-e-Backlog-Pendente-v1.1.md` §6.
...
│   │   ├── CSChip.js
│   │   ├── CSEmptyState.js
│   │   ├── CSGeoStatusCard.js          ← ⚠️ Correção 3.1: NÃO depende de GeofencingTaskService
│   │   ├── CSHeader.js
...
A pasta `src/services/` contém ~~apenas um arquivo: `TokenStorage.js`~~ **três arquivos** — ⚠️ **Correção 3.1 (02/09/2026):** `TokenStorage.js`, `DispositivoId.js` e `NotificacaoPermissao.js` (o último criado na Sprint S8). O diagnóstico de fundo (não é uma camada arquitetural clara, e convive com `*Service.js` espalhados por `screens/`) permanece válido, e o crescimento de 1 para 3 arquivos sem critério declarado até o reforça. Ao mesmo tempo, serviços como `LoginService`, `HospitalService`, `VisitaService` e `FeedbackService` vivem dentro de `screens/`, criando a percepção equivocada de que esses serviços pertencem exclusivamente àquela tela específica.
...
> ⚠️ **Correção 3.1 (02/09/2026):** afirmação **falsa**. `CSGeoStatusCard.js` importa apenas `react`, `react-native`, `CSCard`, `CSButton` e `../theme` — não arrasta `@maplibre/maplibre-react-native` para o bundle. O ponto geral sobre *barrel exports* e a limitação de *tree shaking* do Metro continua de pé; o que cai é **este exemplo**. Antes de dimensionar o ganho da correção, meça o bundle: o impacto pode ser bem menor do que o estimado com base nesta premissa.

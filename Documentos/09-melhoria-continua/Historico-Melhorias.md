...
| **P-004** | ~~O git-flow documentado não é o do repositório~~ — **corrigido em 03/09/2026**, por decisão do PO: *"corrigir para master"*. O fluxo passa a ser `feature/* → develop (dev) → master (produção)`, com `release/<tag>` como versão fixada; o degrau de homologação sai do documento porque não tem branch nem ambiente. 13 referências a `main` corrigidas em 5 workflows e 2 documentos. **Achado no caminho:** o `ci.yml` disparava em `[develop, main]` — como `main` nunca existiu, **nenhum push ou PR para a branch de produção rodava CI**. Uma promoção para produção não era validada por nada. Também entrou uma guarda de credencial no `cd-backend.yml`: sem os secrets do ambiente o deploy **falha com mensagem explícita**, em vez de ficar verde sem publicar — promoção para produção que não entrega é o pior caso do falso-verde. | Auditoria de PRs, 03/09/2026 | ✅ Fechada |

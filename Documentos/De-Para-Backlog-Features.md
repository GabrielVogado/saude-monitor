...
| ID Backlog | Estória | Feature(s) Relacionada(s) | Status Implementação |
|---|---|---|---|
| **E8-01** | Eliminar cold start na primeira abertura | Cross | 🟢 **Destino resolvido — backend no Google Cloud Run desde 04/09/2026** (PR #79, [ADR-011](08-analise%20tecnica/adrs.md)). O Render era a causa: 109 s com HTTP 503 (02/09), 117,9 s e 116 s (03/09). Serviço `saude-monitor-backend-dev` em `southamerica-east1`, revisão `...-00001-kvf`, medido fora da esteira: `/actuator/health` 200 em 0,67 s e `/api/v1/hospitais` 200 em 0,52 s com documento real do Atlas. **Correções de registro:** (a) a mitigação por keep-alive nunca funcionou como este documento afirmava — o agendador do GitHub roda o `cron` ~4x/dia, não as ~96 previstas; o workflow foi **desligado** em 04/09/2026; (b) o **Cloud Run deixou de estar descartado** — a decisão de 03/09 foi revertida pelo PO em 04/09, e desta vez a preparação foi enviada e mergeada (`deploy/google/`, `cd-backend-google.yml`), não apagada. Oracle Cloud segue descartada: São Paulo sem capacidade Always Free e região *home* imutável. **Aberto:** `--min-instances=0` por decisão do PO — cold start ainda existe, subir para 1 é o gatilho se houver reclamação. **O que isto NÃO fecha:** o piso por requisição com o serviço quente é o **E8-02**, que só será reavaliado com medição nova sobre o Cloud Run |

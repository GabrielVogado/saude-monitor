...
| ID Backlog | Estória | Feature(s) Relacionada(s) | Status Implementação |
|---|---|---|---|
| **E8-01** | Eliminar cold start na primeira abertura | Cross | 🟡 **Mitigado, destino indefinido** — medido em 109 s com HTTP 503 (02/09), 117,9 s e 116 s (03/09). **Mitigação ativa desde 03/09/2026:** o `keep-alive-backend.yml` mantém a instância do Render acordada das 07h às 22h. Ele existia desde 02/09 mas **nunca havia executado** — o `schedule` só dispara a partir da branch padrão, que era a `master`, e a API devolvia 404; passou a valer quando a branch padrão virou `develop`. Comprovado: run com `tentativa 1: http=503 em 116s` seguida de `tentativa 2: http=200 em 0s`. **Destino da migração: em aberto.** Oracle Cloud descartada na prática — São Paulo sem capacidade Always Free, e a região *home* da conta é imutável. **Google Cloud Run descartado por decisão do PO em 03/09/2026**, fora de escopo; a branch com os 626 linhas de preparação foi apagada sem nunca ter sido enviada ao `origin`. O PO pesquisará hospedagens. **O que a mitigação NÃO resolve:** o piso de 1 a 5 s por requisição com o serviço quente, que é fração de vCPU compartilhada. Medido em 03/09: com o payload da listagem 72,3% menor (E8-03), o TTFB não se moveu. Isso é o **E8-02**, e só a troca de hospedagem fecha |
